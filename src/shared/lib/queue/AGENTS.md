...
├── sequence.ts         # Redis 기반 순서 보장 (댓글 체이닝)
└── handlers/
    ├── index.ts        # barrel
...
| 작업 중복 방지 | `index.ts` | `generateJobId(data)` — content hash 기반 |
...
| 순서 대기 | `sequence.ts` | `waitForSequenceTurn()` — Redis polling |
| 순서 진행 | `sequence.ts` | `advanceSequence()` — Redis incr |

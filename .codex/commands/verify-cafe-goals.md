...
- source of truth는 실제 네이버 카페의 writer 계정별 `내가 쓴 글` 목록입니다.
- 구현 기준은 `g_sUserMemberKey -> /f-e/cafes/{cafeId}/members/{memberKey} -> CafeMemberNetworkArticleListV3` 흐름입니다.
...
- 전일 PASS/FAIL/UNKNOWN
- 금일 PASS/FAIL/UNKNOWN
- 목표 건수 / 실제 건수
- 부족/초과 차이
- writer identity 확인 개수
- 관측 날짜 범위와 수집 종료 사유
...
- `PASS`: 실제 발행 건수 >= 목표치
- `FAIL`: 실제 발행 건수 < 목표치
- `UNKNOWN`: writer 계정 일부 로그인 실패, 추가 인증 대기, 또는 수집 오류 때문에 최종 단정이 불가능함
...
- 실제 카페 `members/{memberKey}` 화면과 게시글 URL 노출 여부를 확인

...
const KEYWORDS: KeywordEntry[] = [
  { category: '임신준비', keyword: '생리주기 불규칙' },
  { category: '임신준비', keyword: '활성형 엽산' },
  { category: '임신준비', keyword: '40살 임신 확률' },
];
...
const generateOne = async (entry: KeywordEntry, idx: number) => {
  ...
  console.log(`  [${idx}] 앵글: ${angleType} | 경로: ${hasRoute ? '있음' : '없음'}`);
